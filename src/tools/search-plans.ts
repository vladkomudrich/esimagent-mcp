...
export async function searchPlans(args: { country: string }): Promise<string> {
  ...
  try {
    const plans = await fetchAPI<Plan[]>("/plans", { country: countryCode });
    ...
    for (const plan of limited) {
      const bestValue = plan.isBestValue ? " [BEST VALUE]" : "";
      lines.push(`--- ${plan.providerName}${bestValue} ---`);
